...
export class SongEditor {
	...
	private readonly _optionsMenu: HTMLSelectElement = select({style: "width: 100%;"},
		...
		option({value: "colorTheme"}, "Change Theme"),
		option({value: "recordingSetup"}, "Set Up Note Recording..."),
	);
	private readonly _scaleSelect: HTMLSelectElement = buildOptions(select(), Config.scales.map(scale=>scale.name));
	private readonly _keySelect: HTMLSelectElement = buildOptions(select(), Config.keys.map(key => key.name).reverse());
	private readonly _tempoSlider: Slider = new Slider(input({ style: "margin-left: 0.5em; width: 0; vertical-align: left;", type: "range", min: "0", max: "0", value: "0", step: "0" }), this.doc, (oldValue: number, newValue: number) => new ChangeTempo(this.doc, oldValue, Math.round(120.0 * Math.pow(2.0, (-4.0 + newValue) / 9.0))));
	private readonly _tempoStepper: HTMLInputElement = input({style: "width: 4em; margin-left: 3em; vertical-align: middle; text-align: center;", type: "number", step: "1"});
	...
	public whenUpdated = (): void => {
		...
		const optionCommands: ReadonlyArray<string> = [
			...
			"　Choose Layout...",
			(prefs.colorTheme == "death" ? " " : " ") + "Change Theme",
	...
	private _optionsMenuHandler = (event:Event): void => {
		switch (this._optionsMenu.value) {
			...
			case "layout":
				...
				break;
			case "colorTheme":
				this.doc.prefs.colorTheme = this.doc.prefs.colorTheme == "death" ? "dark classic" : "death";
